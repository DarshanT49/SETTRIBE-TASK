...
import { Save, Star, AlertCircle, FileText, Plus, X, Check, ClipboardList, MessageSquare } from 'lucide-react';
 ...
 *
 * Props:
 *   meeting      – interview / meeting data object
 *   currentUser  – logged-in user
 *   onSaved      – called with the payload after successful submit
 *   mode         – "live"  → only scoring section (during interview, no submit)
 *               – "post"  → full form with feedback + recommendation + submit (after interview ends)
 *               Defaults to "post" so existing usages (InterviewDetail page) are unchanged.
 */
export default function InterviewEvaluationPanel({ meeting, currentUser, onSaved, mode = 'post' }) {
  ...
  // Auto-save draft (always, in both modes)
  useEffect(() => {
  ...
  const handleMarkChange = (criterion, value) => {
    setForm(prev => ({ ...prev, marks: { ...prev.marks, [criterion]: Number(value) } }));
  };
  ...
  const handleRemoveSkill = (skill) => {
    setForm(prev => {
      ...
      return { ...prev, criteria: prev.criteria.filter(c => c !== skill), marks: newMarks };
    });
  };
  ...
  // ─── Live mode header label ───────────────────────────────────────────────
  const headerTitle = mode === 'live' ? 'Live Scoring' : 'Interview Evaluation';
  const headerSub =
    mode === 'live'
      ? `${meeting?.round || meeting?.interviewType || 'General'} Round — Score skills in real-time (0–5)`
      : `${meeting?.round || meeting?.interviewType || 'General'} Round — Submit after the interview ends`;
    ...
    <div className="bg-gray-900 border-l border-gray-800 flex flex-col h-full overflow-hidden w-full">
      ...
      <div className="p-4 border-b border-gray-800 shrink-0 bg-gray-800/50 flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1 capitalize">{headerSub}</p>
        </div>
        {saveStatus && (
      ...
      {/* ── Scrollable body ── */}
      <div className={`p-4 overflow-y-auto flex-1 space-y-6 ${mode === 'post' ? 'pb-20' : 'pb-4'}`}>
        ...
        {/* Tech Stack & Skills Scoring — shown in BOTH modes */}
        <div className="space-y-3">
          <div className="flex items-center justify-between border-b border-gray-800 pb-2">
            <h3 className="text-sm font-semibold text-gray-200">Tech Stack &amp; Skills</h3>
            {mode === 'live' && (
              <span className="text-[10px] text-primary-400 bg-primary-900/30 px-2 py-0.5 rounded-full font-medium">
                Score in real-time
              </span>
            )}
          </div>
            ...
            {form.criteria.map(criterion => (
              <div key={criterion} className="flex items-center justify-between p-2.5 bg-gray-800/30 rounded-lg border border-gray-800 group">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={() => handleRemoveSkill(criterion)}
        ...
        {/* ── Interview Feedback — ONLY in post mode ── */}
        {mode === 'post' && (
          <>
            <div className="space-y-4 pt-2">
              <h3 className="text-sm font-semibold text-gray-200 border-b border-gray-800 pb-2 flex items-center gap-2">
            ...
            <div className="flex items-start gap-2 p-3 bg-blue-900/20 text-blue-400 rounded-lg text-xs">
              ...
              <p>Your scores are auto-saved. Complete the feedback above, then submit to notify HR.</p>
            </div>
          </>
        )}
        ...
        {/* ── Live-mode tip ── */}
        {mode === 'live' && (
          <div className="flex items-start gap-2 p-3 bg-amber-900/20 text-amber-400 rounded-lg text-xs">
            <AlertCircle size={14} className="shrink-0 mt-0.5" />
            <p>Scores are auto-saved. After the interview ends you'll be prompted to add feedback and submit the full evaluation to HR.</p>
          </div>
        )}
      </div>

      {/* ── Submit button — ONLY in post mode ── */}
      {mode === 'post' && (
        <div className="p-4 border-t border-gray-800 shrink-0 bg-gray-900 absolute bottom-0 left-0 right-0">
          <Button loading={loading} onClick={handleSubmit} className="w-full h-11">
            <Save size={18} className="mr-2" /> Submit Evaluation to HR
          </Button>
        </div>
      )}
    </div>
